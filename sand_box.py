...
def contact_test_by_decay(frg_inf, pos_crd, bin_bnd, n_perm=1000, verbose=True, sigma=0):
    ...
    if n_bin < 50:
        ...
        for bi in range(n_bin):
            ...
            for rd_idx in hit_lst:
                cvg_mat[bi, flatten(cfb_lst[rd_idx])] += 1
    ...
    neg_lst = range(n_neg)
    pos_rdx = np.random.randint(n_pos, size=[n_perm, n_pos])
    for ei in np.arange(n_perm):
        ...
        for ni in range(n_pos):
            ...
            # prf_rnd[ei, flatten(f2b_rnd)] += 1  # making sure one element is randomly ignored everytime

            prf_rnd[ei, neg_fbdx[ni]] += 1

            # np.random.shuffle(cfb_pos[pos_rdx[ei, ni]])
            # prf_rnd[ei, cfb_pos[pos_rdx[ei, ni]][0]] += 1
            # prf_rnd[ei, cfb_pos[pos_rdx[ei, ni]][1]] += 1
